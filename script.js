document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("triangulate").addEventListener("click", async () => {
        if (!complete) {
        ...
        drawTrianglesWithDelay(triangles);
        triangulatedFeatures = triangles.features;
    });

    async function drawTrianglesWithDelay(triangles) {
        for (const feature of triangles.features) {
            const triangleCoords = feature.geometry.coordinates[0];
            const trianglePoints = triangleCoords.slice(0, 3).map((coord) => ({
                x: coord[0],
                y: coord[1],
            }));
    ...
    document.getElementById("completePloy").addEventListener("click", () => {
        ...
        if (!lineIntersectsPolygon(firstPoint, lastPoint, points, false)) {
            ...
            appendAlert(
                "Cannot complete the polygon: the connecting line intersects with another line!",
                "danger"
            );
        }
    });
    ...
    document.getElementById("runAGP").addEventListener("click", () => {
        ...
        const triangulatedCoords = triangulatedFeatures.flatMap((feature) =>
    ...
    document.getElementById("gaurd").addEventListener("click", () => {
        ...
        polygonVertex.forEach((vertex) => {
            const currentColor = vertexColorMap.get(
                JSON.stringify([vertex.x, vertex.y])
            );
    ...
    function isTriangleAdjacent(triangle1, triangle2) {
        const sharedVertices = triangle1.filter((vertex1) =>
            triangle2.some(
                (vertex2) => vertex1[0] === vertex2[0] && vertex1[1] === vertex2[1]
            )
        );
    ...
    function drawPoint(point) {
        ...
        gradient.addColorStop(0, "#332615");
        gradient.addColorStop(1, "#332615");
